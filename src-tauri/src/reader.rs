use accounts::account::{Entry, Side, Transaction, TransactionStatus};
...
impl ColumnTypes {
    pub fn index_of(&self, column: ColumnType) -> usize {
        self.columns
            .iter()
            .position(|c| c == &column)
            .unwrap_or(ColumnType::Unknown as usize)
    }
    ...
    pub fn from_vec(v: Vec<String>) -> ColumnTypes {
        ColumnTypes {
            columns: v.iter().map(|c| ColumnType::from_str(c).unwrap()).collect(),
        }
    }
...
pub fn check_csv_format<P: AsRef<Path>>(
    path: &P,
    reverse_dr_cr: bool,
) -> Result<ColumnTypes, BooksError> {
...
pub fn check_date_format(rows: &Vec<Vec<String>>, date_column: usize) -> Option<String> {
    ...
    const CANDIDATE_FORMATS: [&str; 18] = [
        "%d/%m/%Y", "%m/%d/%Y", "%Y/%m/%d", "%d-%m-%Y", "%m-%d-%Y", "%Y-%m-%d", "%d.%m.%Y",
        "%m.%d.%Y", "%Y.%m.%d", "%d/%m/%y", "%m/%d/%y", "%y/%m/%d", "%d-%m-%y", "%m-%d-%y",
    ...
    for row in rows {
        ...
        let Some(token) = extract_date_token(cell) else {
            continue;
        };
        ...
        considered += 1;
        let mut parsable: std::collections::HashSet<&'static str> =
...
pub fn read_rows<P: AsRef<Path>>(
    path: &P,
    reverse_dr_cr: bool,
) -> Result<Vec<Vec<String>>, BooksError> {
    let columns = read_columns(path, reverse_dr_cr)?;
    let _ = validate_columns(&columns);
    let rdr = csv::ReaderBuilder::new().has_headers(false).from_path(path);
    ...
    match rdr {
        ...
        Err(e) => return Err(crate::books_error!("errors.read_csv_failure", error => e)),
    }
}
...
fn detect_columns(headers: &StringRecord, reverse_dr_cr: bool) -> Result<ColumnTypes, BooksError> {
    ...
    for header in headers {
        match header.trim().to_lowercase().as_str() {
            ...
            "debit" => {
                if reverse_dr_cr {
                    columns.push(ColumnType::Credit)
                } else {
                    columns.push(ColumnType::Debit)
                }
            }
            "credit" => {
                if reverse_dr_cr {
...
fn validate_columns(columns: &ColumnTypes) -> Result<(), BooksError> {
    ...
    if columns.len() >= 3
        && (columns.has_exactly_one_of(ColumnType::Amount)
            || (columns.has_exactly_one_of(ColumnType::Debit)
                && columns.has_exactly_one_of(ColumnType::Credit)))
    {
        return Ok(());
    }
...
pub fn transactions_are_reversed(transactions: &[Transaction], account_id: Uuid) -> bool {
    ...
    for window in transactions.windows(2) {
        if let (Some(first_entry), Some(second_entry)) = (
            ...
            if first_entry.date > second_entry.date
                || (first_entry.date == second_entry.date && currently_decreasing)
            {
                decreasing_count += 1;
    ...
    println!(
        "decreasing_count: {}, total_comparisons: {}",
        decreasing_count, total_comparisons
    );
...
fn parse_money_str(amount: String) -> Result<Decimal, BooksError> {
    ...
    match Decimal::from_str(&amount_str) {
        Ok(amount) => Ok(amount),
        Err(e) => {
            Err(crate::books_error!("errors.parse_amount_with_value", amount => amount, error => e))
        }
    }
}
...
fn parse_date_str(date_str: String, format: &str) -> Result<NaiveDate, BooksError> {
    match NaiveDate::parse_from_str(&date_str, format) {
        ...
        Err(e) => {
            return Err(
                crate::books_error!("errors.parse_date_with_format", date => date_str, format => format, error => e),
            )
        }
    };
}
...
mod tests {
    ...
    fn test_parse_date_str() {
        assert_eq!(
            ...
            parse_date_str("20/10/2023".to_string(), "%d/%m/%Y").unwrap()
        );
        assert_eq!(
            NaiveDate::from_ymd_opt(2023, 10, 20).unwrap(),
            parse_date_str("2023-10-20".to_string(), "%Y-%m-%d").unwrap()
        );
        ...
        assert_eq!(
            "Unable to parse date '2023-13-20' using format '%Y-%m-%d': input is out of range",
            parse_date_str("2023-13-20".to_string(), "%Y-%m-%d")
                .unwrap_err()
                .error
                .as_str()
        );
    ...
    fn test_reader_file_not_found() {
        ...
        let result = read_transactions(
            &String::from("no_such_file.csv"),
            account.id,
            "%d/%m/%Y",
            &columns,
            true,
        ...
        assert_eq!(
            "Unable to read csv file. No such file or directory (os error 2)",
    ...
    fn test_reader() {
        let account = Account::create_new("Savings Account 1", AccountType::Asset);
        let columns = read_columns(&"test.csv", true).unwrap();
        let transactions = read_transactions(
